...
$(builds):
	-docker build -f Dockerfile.$@ --pull --tag $(ACCOUNT)/$@:$(VERSION) .

.PHONY: clean $(src)
clean: $(subst Dockerfile.,,$@) $(clean_builds)
$(clean_builds):
	-@docker rmi $(ACCOUNT)/$(subst clean_,,$@):$(VERSION)
...
$(clean_all_builds):
	...
	-@docker rmi $(ACCOUNT)/$(subst cleanall_,,$@):$(VERSION)

.PHONY: push $(src)
push: $(subst Dockerfile.,,$@) $(push_builds)
$(push_builds):
	-@docker push $(ACCOUNT)/$(subst push_,,$@):$(VERSION)
